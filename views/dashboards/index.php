            ...
            <div class="card-header pb-0">
              ...
              <p class="text-sm">
                <i class="fa fa-arrow-up text-success" aria-hidden="true"></i>
		            <span class="font-weight-bold"><?php echo $pending; ?></span> Pending
	            </p>
            </div>
	          <div class="card-body p-3">
              <div class="timeline timeline-one-side">
                <?php foreach(json_decode($topups, true) as $topup) { ?>      
                  <div class="timeline-block mb-3">
                    <span class="timeline-step">
	                    <i class="ni <?php echo !isset($topup['type']) ? '' : $topup['type'] == 'Refund' ? 'ni-credit-card text-danger' : 'ni-bell-55 text-success' ?> text-gradient"></i>
                    </span>
	                  <div class="timeline-content">
 	                    <span class="badge badge-sm <?php echo !isset($topup['type']) ? '' : $topup['type'] == 'Refund' ? 'bg-gradient-secondary' : 'bg-gradient-success' ?>" style="float: right;"><?php echo $topup['type'] ?></span>
	                    <h6 class="text-dark text-sm font-weight-bold mb-0"><?php echo ucwords(strtolower($topup['currency'])) . ' ' . number_format($topup['amount']) ?></h6>
                      <p class="text-secondary font-weight-bold text-xs mt-1 mb-0"><?php 
                        $date = new DateTime();
                        $date->setTimezone(new DateTimeZone('Africa/Nairobi'));
                        $date->setTimestamp($topup['createTime']/1000);
                        echo strtoupper($date->format('d M H:m A'));
	                    ?>
                      </p>
                    </div>
                  </div>
