let merchantRef = db.collection('b2b').doc('ke').collection('businesses');
let b2bPos = {};
let deleteIDs = [];
	...
	data.docs.forEach(doc => {
		...
		let item =
			`<tr data-id="${doc.id}">
				...
				<td class="text-dark text-sm font-weight-small mb-0">${merchant.businessname}</td>
				...
				<td class="text-dark text-sm font-weight-small mb-0">${merchant.phone}</td>
                		<td class="text-dark text-sm font-weight-small mb-0">${merchant.about}</td>
				...
				<td>
				<a href="/merchants/view?id=${doc.id}&name=${merchant.businessname}&email=${merchant.email}&phone=${merchant.phone}" id="${doc.id}" class="edit js-view-merchant"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
		...
		db.collection('b2b').doc('ke').collection('businesses').doc(id).get().then(function (document) {
			if (document.exists) {
				...
				$('#edit-merchant-form #firstname').val(document.data().firstname);
                                $('#edit-merchant-form #imageid').val(document.data().imageid);
...
{/* <a href="#" id="${doc.id}" class="delete js-delete-merchant"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a> */}
function openForm() {
